            ...
            >
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    {items.length === 0 ? (
                        ...
                        items.map((item) => (
                            <div key={item.cartItemId} className="flex gap-4 p-4 bg-white rounded-2xl border border-gray-100 shadow-sm relative group hover:border-[#2D5C35]/30 transition-colors">
                                <div className="relative w-20 h-20 bg-gray-50 rounded-xl overflow-hidden flex-shrink-0">
                                    <Image src={item.image} alt={item.name} fill className="object-cover" />
                                </div>
                                <div className="flex-1">
                                    <div className="flex justify-between items-start mb-1">
                                        <h4 className="font-bold text-sm text-[#1F2937] line-clamp-2 pr-6">{item.name}</h4>
                                        <button
                                    ...
                                    <p className="text-xs text-[#2D5C35] font-bold mb-3">{item.selectedSize}</p>
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <p className="font-bold text-[#1F2937]">₹{item.selectedPrice * item.quantity}</p>
                                    </div>
                                </div>
                            </div>
                        ))
                    ...
                    <div className="p-6 bg-white border-t border-gray-100 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                        ...
                        <div className="space-y-2 mb-6 text-sm">
                            <div className="flex justify-between text-gray-500">
                                <span>Subtotal</span>
                                <span className="font-bold text-[#1F2937]">₹{cartTotal}</span>
                            </div>
                            <div className="flex justify-between text-gray-500">
                                <span>Shipping</span>
                                <span className="font-bold text-green-600">{remainingForFreeShipping === 0 ? "FREE" : "₹50"}</span>
                            </div>
                            {couponApplied && (
                            ...
                            <div className="flex justify-between text-lg font-bold text-[#1F2937] pt-4 border-t border-dashed border-gray-200">
                                <span>Total</span>
                                <span>₹{cartTotal + (remainingForFreeShipping === 0 ? 0 : 50) - (couponApplied ? 100 : 0)}</span>
                            </div>
                        </div>

                        <button className="w-full bg-[#2D5C35] text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-[#234a2b] shadow-lg shadow-green-900/20 transition-all group">
